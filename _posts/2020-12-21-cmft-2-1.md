...
이 연산자는 단순히 $\lambda$ 상태에 들어있는 입자의 수를 세는 역할을 한다. $\hat o = \sum_i o_{\lambda_i}ert\lambda_i\rangle \langle\lambda_iert,$  $o_{\lambda_i} = \langle\lambda_iert\hat oert\lambda_j \rangle$인, 다시 말해서 기저$ert\lambda \rangle$로 대각화 되어있는 단입자 연산자 $\hat \mathcal {O}_1$을 생각하자. 정의에 의해 다음을 만족함을 알 수 있다.
