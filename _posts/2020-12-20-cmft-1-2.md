...
$$\langle \mathbf f, \mathbf g \rangle = \sum_{n=1}^N f_ng_n \rightarrow \langle f,g \rangle = \int dx f(x)g(x)
\\
f_n = \langle \mathbf f,\mathbf e_n \rangle = \sum_m f_m(e_n)_m \rightarrow f(x) = \langle f,\delta_x \rangle = \int dx' f(x')\delta_x(x')$$
